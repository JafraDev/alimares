<?php
    $decode_data = $dataResult->getPerfiles();
    echo "<div class='row'>";
    echo "<div class='col-md-12 tableFixHead'>";
    echo "<table class='table table-stripped'>";
    echo "<thead>";
    echo "<tr>";
    echo "<th>ID</th>";
    echo "<th>Perfil</th>";
    echo "<th>";
    echo "</th>";
    echo "</tr>";
    echo "</thead>";
    echo "<tbody>";
    foreach($decode_data as $key=>$value){
        echo "<tr>";
        echo "<td><a href='#' 
        onclick='
        document.getElementById(\"nombre_perfil\").value = (\"".$decode_data[$key]['nombre_perfil']."\");
        document.getElementById(\"perfil\").readOnly = true;
        document.getElementById(\"id\").value = (this.parentElement.innerText); 
        document.getElementById(\"id_reg\").innerText = (this.parentElement.innerText); 
        document.getElementById(\"perfil\").value = (this.parentElement.innerText)'>".$decode_data[$key]['perfil']."</a></td>";
        echo "<td>".$decode_data[$key]['nombre_perfil']."</td>";
        echo "<td>";
        echo "<img style=\"display:none;\" id=\"setDocsIcon\" onClick=\"openFormSetDocs('".$decode_data[$key]['nombre_perfil']."'," .$decode_data[$key]['perfil'].")\" src=\"data:image/png;base64,\">";
        echo "</td>";
        echo "</tr>";
    }      
    echo "</tbody>";
    echo "</table>";
    echo "</div>";
    echo "</div>";
?>